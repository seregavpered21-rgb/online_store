...
- Next.js 16-приложение с немецкой витриной, каталогом, категориями, поиском и сортировкой товаров.
- Карточки товаров показывают варианты, актуальную цену, остаток, изображения из Vercel Blob и SEO-метаданные.
- Гостевая корзина хранится в браузере. После входа она объединяется с серверной корзиной; восстановление локальной корзины не вызывает React hydration error.
- Checkout принимает контакты, выбор Essen или почтовой доставки и адрес для почтовой отправки. Первая версия использует оплату при получении.

### Данные, пользователи и заказы

- Drizzle-схемы и миграции для каталога, клиентов, корзин, заказов и Better Auth применены к Neon/Postgres.
- Регистрация, вход, выход, роль `customer`, ручное назначение `admin` и защищённые account/admin-маршруты работают.
- Заказ создаётся одним атомарным SQL-оператором: наличие проверяется на сервере, остатки списываются только при успешном создании заказа, а цена и названия позиций сохраняются в заказе.
- Оформленный из браузера тестовый заказ прошёл цепочку товар -> корзина -> checkout -> подтверждение. Оплата в этом сценарии отмечается как расчёт при получении.
- Авторизованный покупатель видит связанные заказы в `Konto`. Администратор может редактировать товары и изображения, просматривать заказы, менять допустимые статусы, хранить внутренние заметки и скачивать CSV.

### Качество и эксплуатация

- `npm test` запускает 12 unit-тестов: операции корзины, валидация и консолидация заказа, переходы статусов и проверка роли admin.
- `npm run lint` и `npm run build` успешно выполнялись после добавления тестов.
- Витрина, каталог, корзина и checkout проверены на мобильной ширине `390px`: горизонтального переполнения нет, у полей есть доступные имена, начальная клавиатурная навигация работает.
- Dependabot в [.github/dependabot.yml](.github/dependabot.yml) создаёт еженедельные PR для `npm` и GitHub Actions.
- [Инструкция продавца](docs/SELLER_GUIDE.md) описывает публикацию товаров, загрузку фото и обработку заказов.

## Что проверить перед запуском продаж

## Статус MVP

Техническая минимальная версия магазина готова: покупатель может найти товар, зарегистрироваться, собрать корзину и оформить заказ с оплатой при получении. Продавец может управлять каталогом, остатками, изображениями и обработкой заказов.

До публичного запуска для реальных покупателей остаются обязательные операционные шаги: финальный бренд и legal-данные, собственный домен, проверка реального заказа с email и admin-статусом, настройка резервного копирования Neon и проверка production-переменных Vercel.

Онлайн-оплата, автоматические интеграции доставки, синхронизация корзины между устройствами, аналитика, Sentry, пагинация и audit log не блокируют первую версию с оплатой при получении. Их стоит решать отдельными задачами после выбора конкретных сервисов и появления первых реальных сценариев.

### Обязательно до приёма реальных заказов

1. Заполните финальные название, домен, логотип, контакты, реквизиты и юридические тексты. Шаблоны legal-страниц нельзя считать юридической консультацией.
2. Подключите собственный домен в Vercel, включите HTTPS и обновите `BETTER_AUTH_URL`, `NEXT_PUBLIC_APP_URL`, metadata и `EMAIL_FROM`.
3. Настройте Resend: подтвердите sender/domain, добавьте `RESEND_API_KEY` и `EMAIL_FROM` в Vercel, сделайте redeploy. Затем оформите тестовый заказ и убедитесь, что покупатель получил письмо.
4. Войдите в `/admin` под admin-пользователем, откройте тестовый заказ и проверьте смену `new -> confirmed -> fulfilled`; отдельно убедитесь, что отмена доступна только из `new` или `confirmed`.
5. Проверьте реальные товары: названия, цены, SKU, остатки, варианты, фотографии, alt-тексты, стоимость/сроки доставки и наличие товара после оформления заказа.
6. Выберите платёжный сервис до включения предоплаты. Сейчас система принимает только заказы с оплатой при получении и не проверяет платёжные webhooks.

### Операционная проверка

1. В Neon проверьте текущую политику backup/PITR, выполните или отрепетируйте восстановление и выдайте команде минимально необходимые доступы. Эта настройка выполняется в кабинете Neon и зависит от тарифа.
2. В Vercel убедитесь, что production-переменные заданы для нужных окружений и не содержат тестовых значений. После каждой замены секретов выполняйте redeploy.
3. Просматривайте Dependabot PR еженедельно. Перед объединением обновлений запускайте `npm test`, `npm run lint` и `npm run build`.
4. Добавьте аналитику с cookie consent и мониторинг ошибок после выбора провайдеров и создания их проектов.

### Что ещё не покрыто автоматически

- Серверная корзина ещё не загружается обратно в интерфейс после входа и не синхронизируется между устройствами.
- Реальная отправка email не проверена, пока Resend не настроен.
- Полный UI-сценарий admin-статуса требует входа в существующий admin-аккаунт.
- Резервное копирование Neon, доступы команды, домен, платежи, аналитика и Sentry требуют внешних аккаунтов и не могут быть включены только изменением исходного кода.
