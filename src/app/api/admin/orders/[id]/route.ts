...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  if (body.status !== undefined && !Object.hasOwn(transitions, body.status)) return NextResponse.json({ error: "Ungültiger Bestellstatus." }, { status: 400 });
  ...
  if (body.status !== undefined && !transitions[order.status].includes(body.status)) return NextResponse.json({ error: "Dieser Statuswechsel ist nicht erlaubt." }, { status: 409 });
